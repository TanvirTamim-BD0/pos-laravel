...
@section('content')
    ...
    @section('sell.index') active @endsection
@section('styles')
@endsection
@section('content')
                    ...
                    <div class="card-header border-0 pt-6">
                        ...
                        <div class="card-toolbar">
                            ...
                            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                                ...
                                <!--end::Export-->
                                <!--begin::Add customer-->
                                <a href="{{ route('sell.create') }}" class="btn text-white"
                                    style="background-color: #2F4F4F"><i class="las la-shopping-cart fw-bolder"></i> POS</a>
                                <!--end::Add customer-->
                            </div>
                            <!--end::Toolbar-->
                                ...
                                <table class="table align-middle table-row-dashed gy-5 dataTable no-footer"
                                    ...
                                    <tbody class="fs-6 fw-bold text-gray-600">
                                        @foreach ($sellData as $sell)
                                            <tr>
                                                <td>
                                                    <a href="{{route('selling-invoice', $sell->id)}}" class="text-success fw-bolder text-decoration-underline">
                                                        {{ $sell->selling_id }}
                                                    </a>
                                                </td>
                                                <td>
                                                    <a class="fw-bolder text-decoration-underline">
                                                        {{ $sell->customerData->customer_name }}
                                                    </a>
                                                </td>
                                                <td>
                                                    <span class=" fw-bolder">Prodcts: </span>
                                                    @foreach ($sell->sellProductData as $product)
                                                    <span class=" badge-light-primary">{{ $product->productData->product_name }} </span> ,
                                                    @endforeach
                                                </td>

                                                <td>{{ number_format($sell->sellPaymentData->paid_amount) }} </td>
                                                <td>{{ number_format($sell->sellPaymentData->due_amount) }} </td>

                                                @php
                                                    $sellingProductPrice = App\Models\SellPayment::getSellingProductPrice($sell->id);   
                                                @endphp

                                                <td>
                                                    <span class="fw-bolder">
                                                        {{ number_format($sellingProductPrice) }} 
                                                    </span>
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4"
                                                        data-kt-menu="true">
                                                         ...
                                                         <div class="menu-item">
                                                            <a href="{{route('selling-invoice', $sell->id)}}"
                                                        ...
                                                        <div class="menu-item">
                                                            ...
                                                            <form method="POST"
                                                                action="{{ route('sell.destroy', $sell->id) }}"
                                                                class="menu-link">
                                                                @csrf
                                                                @method('delete')
